...
if(!empty($phone_no))$phone_no = '92'.substr($phone_no, -10); 
$objuser->m_phone_no = $phone_no;

$fax_no = str_replace(' ', '', $fax_no);
if(!empty($fax_no))$fax_no = '92'.substr($fax_no, -10); 
...
if ($strDo == "Add") {
    /* $sql = "select usrlogin_id from sysuser_tab where usrlogin_id = '" . $usrlogin_id . "'";
      ...
      header("location:ManageUserUSSD.php?msg=username+is+not+available");
      exit;
      }
      //If warehouses is not assigned to user
      if (empty($wh_id[0])) {
      header("location:ManageUserUSSD.php?msg=Warehouse+not+selected");
      exit;
      } */
...
header("location:ManageUserUSSD.php");
exit;
?>