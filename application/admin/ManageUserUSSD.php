...
if ($strDo == "Delete") {
    ...
    //Redirecting to ManageUserProv
    echo '<script>window.location="ManageUserUSSD.php"</script>';
    exit;
}
/**
 * Edit
...
<body class="page-header-fixed page-quick-sidebar-over-content" <?=($strDo!='Edit'?'onLoad="doInitGrid()"':'')?>>
    ...
    <div class="page-container">
        ...
        <div class="page-content-wrapper">
            <div class="page-content">
                <div class="row">
                    <div class="col-md-12">
                        <h3 class="page-title row-br-b-wp">USSD User Management</h3>
                        <div class="widget" data-toggle="collapse-widget">
                            <div class="widget-head">
                                <h3 class="heading"><?php echo str_replace('_',' ',ucwords($strDo)); ?> User</h3>
                            </div>
                            <div class="widget-body">
                                <form method="post" action="ManageUserUSSDAction.php" name="manageuser" id="manageuser" enctype='multipart/form-data'>
                                                        ...
                                                        <select name="select" id="Stakeholders" class="form-control input-medium">
                                                            ...
                                                            if ($rsStakeholders != FALSE && mysql_num_rows($rsStakeholders) > 0) {
                                                                while ($RowGroups = mysql_fetch_object($rsStakeholders)) {
                                                                    ...
                                                                    {
                                                                        if($RowGroups->stkid != $_SESSION['user_stakeholder1']) continue;
    ...
    <script>
        function activateFunction(val) {
            //window.location = "ManageUserUSSD.php?Do=activate&Id=" + val;
        }
        function deactivateFunction(val) {
           // window.location = "ManageUserUSSD.php?Do=deactivate&Id=" + val;
        }
        function editFunction(val) {
            window.location = "ManageUserUSSD.php?Do=Edit&Id=" + val;
        }
        function changePasswordFunction(val) {
            window.location = "ManageUserUSSD.php?Do=Change_Password&Id=" + val;
        }
        function delFunction(val) {
            if (confirm("Are you sure you want to delete the record?")) {
                window.location = "ManageUserUSSD.php?Do=Delete&Id=" + val;
            }
        }
        var mygrid;
        function doInitGrid() {
            ...
            mygrid.setHeader("<span title='Serial Number'>Sr. No.</span>,<span title='Stakeholder'>Stakeholder</span>,<span title='Province'>Province</span>,<span title='District'>District</span>,<span title='Warehouse'>Warehouse</span>,<span title='Phone No'>Phone Number</span>,<span title='Username'>Username</span>,<span title='Role'>Role</span>,<span title='Use this column to perform the desired operation'>Actions</span>,#cspan,#cspan");
            mygrid.attachHeader(",#select_filter,#select_filter,#text_filter,#text_filter,#text_filter,#text_filter,#select_filter");
            mygrid.setInitWidths("50,80,80,80,*,110,110,90,30,30,30");
